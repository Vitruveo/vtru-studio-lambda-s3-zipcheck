...
const notify = async ({ filename, bucket, region }) => {
    // TODO: marcos, please implement that 😆
};

const md5hash = (value) => {
    return cryto.createHash('md5').update(value).digest('hex');
};

const checkFile = async ({ filename, bucket, region }) => {
    const s3 = new s3client.S3Client({ region: region });
    const params = {
        Bucket: bucket,
        Key: filename,
    };
